    ...
    methods:{
        ...
        getusers: function () {
            this.$post('users/profile', { token: this.$store.state.id }).then(res => {
                ...
                this.users.name?document.querySelector('#name').setAttribute("readonly","readonly"):this.users.name;
                this.users.company?document.querySelector('#unit').setAttribute("readonly","readonly"):this.users.unit;
                this.users.email?document.querySelector('#email').setAttribute("readonly","readonly"):this.users.email;   
            })
        },
        savePersonal: function () {
            ...
            this.$post('users/edit', { token: this.$store.state.id, name: this.users.name, email: this.users.email, company: this.users.company, idcard: this.users.idcard }).then(res => {
                ...
                if (res.data.status === 'fail') {
                    ...
                    this.message='修改成功';
                    this.$store.state.name=this.users.name;
                    let model = document.querySelector('#modal-close-default-two');
                    UIkit.modal(model).show();
